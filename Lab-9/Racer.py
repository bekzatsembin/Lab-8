...
class Coin(pygame.sprite.Sprite):
      ...
      def move(self):
          ...
          if (self.rect.bottom > 600):
            self.rect.center = (random.randint(40,SCREEN_WIDTH-40), random.randint(40,SCREEN_WIDTH-40))
          #Checking if the player touches coin, then it will collect it
...
incsp = pygame.USEREVENT + 2

if pygame.sprite.spritecollideany(P1, coins):
        collision_counter += 1
        if collision_counter == 5:
            # Вызов события, которое происходит каждые 5 столкновений
...
while True:
    ...
    for event in pygame.event.get():
        if event.type == INC_SPEED or event.type == incsp:
              SPEED += 0.5      
        if event.type == QUIT:
            pygame.quit()
            sys.exit()
